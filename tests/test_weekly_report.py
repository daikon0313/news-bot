...
# ヘルパー
# ---------------------------------------------------------------------------
def _create_posted_files(base_dir, files_data):
...
class TestWeeklyReport:
    def test_report_with_data(self, tmp_path, capsys):
        ...
        _create_posted_files(tmp_path, [
            ("posted_morning_2026-02-03.json", tweets_mon),
            ("posted_evening_2026-02-04.json", tweets_tue),
        ])

        with patch("sys.argv", ["weekly_report.py", "2026-02-02", "2026-02-09"]):
            os.chdir(tmp_path)
            weekly_report.main()
    ...
    def test_report_date_filtering(self, tmp_path, capsys):
        ...
        _create_posted_files(tmp_path, [
            ("posted_morning_2026-02-05.json", tweets),       # 範囲内
            ("posted_morning_2026-01-20.json", out_of_range), # 範囲外
            ("posted_morning_2026-02-15.json", out_of_range), # 範囲外
        ])
    ...
    def test_category_distribution(self, tmp_path, capsys):
        ...
        _create_posted_files(tmp_path, [
            ("posted_morning_2026-02-05.json", tweets),
        ])
    ...
    def test_malformed_json_file(self, tmp_path, capsys):
        ...
        posted_dir.mkdir(parents=True, exist_ok=True)
        (posted_dir / "posted_morning_2026-02-05.json").write_text("not valid json")
